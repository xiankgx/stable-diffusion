...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
                 ...
                 # GX Note: parameterizatio and loss
                 parameterization="eps",  # all assuming fixed variance schedules
                 loss_type="l2",

                 # GX Note: checkpointing
                 ckpt_path=None,
                 ignore_keys=[],
                 ...
                 # GX Note: clip denoised here is ok because we are working in image space with values in [-1, 1]
                 clip_denoised=True,
...
class LatentDiffusion(DDPM):
    ...
    def make_cond_schedule(self, ):
        self.cond_ids = torch.full(
            size=(self.num_timesteps,), fill_value=self.num_timesteps - 1, dtype=torch.long)
        ids = torch.round(torch.linspace(
            0, self.num_timesteps - 1, self.num_timesteps_cond)).long()
    ...
    def register_schedule(self,
        ...
        super().register_schedule(given_betas, beta_schedule,
                                  timesteps, linear_start, linear_end, cosine_s)
    ...
    def get_fold_unfold(self, x, kernel_size, stride, uf=1, df=1):
        ...
        if uf == 1 and df == 1:
            ...
            weighting = self.get_weighting(
                kernel_size[0], kernel_size[1], Ly, Lx, x.device).to(x.dtype)
            ...
            fold = torch.nn.Fold(output_size=(
                x.shape[2] * uf, x.shape[3] * uf), **fold_params2)

            weighting = self.get_weighting(
                kernel_size[0] * uf, kernel_size[1] * uf, Ly, Lx, x.device).to(x.dtype)
            normalization = fold(weighting).view(
            ...
            fold = torch.nn.Fold(output_size=(
                x.shape[2] // df, x.shape[3] // df), **fold_params2)

            weighting = self.get_weighting(
                kernel_size[0] // df, kernel_size[1] // df, Ly, Lx, x.device).to(x.dtype)
            normalization = fold(weighting).view(
    ...
    def get_input(
        ...
        cond_key=None,                      # conditioning key
        ...
        # GX Note: Encode using first stage model, a autoencoder
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False):
        ...
        # GX Note: Inverse the latent rescaling
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            output_list = [self.model(z_list[i], t, **cond_list[i])
                           for i in range(z.shape[-1])]
    ...
    def sample(self, cond, batch_size=16, return_intermediates=False, x_T=None,
        ...
        if cond is not None:
            if isinstance(cond, dict):
                cond = {
                    key: cond[key][:batch_size] if not isinstance(cond[key], list)
                    else list(map(lambda x: x[:batch_size], cond[key])) for key in cond
                }
            else:
    ...
    def log_images(
        ...
        **kwargs
        ...
        # GX Note: Whether to use DDIM sampling
        ...
        # c     : conditioning
        # x     : input
        # xrec  : reconstruction (from the first stage autoencoder)
        # xc    : original conditioning
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None):
        # GX Note: Various ways to incorporate conditioning into model
